...
class Show extends BaseModel {
  ...
  /**
   * Initializes the Show model instance.
   * Loads any initial data or configurations if necessary.
   * @returns {Show} - An instance of the Show model.
   */
  static async create() {
    const instance = new Show();
    await instance.lookupHelper.loadInitialData(); // Assuming lookupHelper exists in BaseModel
    return instance;
  }

  /**
   * Removes duplicates from an array based on specified fields.
   * @param {Array} array - Array of objects.
   * @param {Array} fields - Fields to base duplication on.
   * @param {Object} options - Options for deduplication.
   * @param {boolean} options.caseInsensitive - Whether to treat fields case-insensitive.
   * @param {string} options.keep - 'first' or 'last' occurrence.
  ...
  /**
   * Processes and prepares show data for insertion.
   * @param {Array} breedingInfoArray - 2D array containing breeding show information.
   * @returns {Array} - An array of show objects ready to be inserted into the database.
   */
