...
exports.show_participant = async (rawDataArray) => {
  try {
    ...
    console.log(`${insertedshow_participant.length} show_participant associations inserted/upserted successfully.`);
    return `${insertedshow_participant.length} show_participant associations inserted/upserted successfully.`;
  } catch (error) {
